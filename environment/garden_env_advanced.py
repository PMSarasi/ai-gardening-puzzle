...
class GardenEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Calculate reward with enhanced shaping
        ...
        if done:
            reward += self._get_episode_bonus()
    ...
    def _calculate_reward(self, plant_idx, action):
        """Enhanced reward function with clear guidance"""
        if plant_idx >= len(self.plants):
            return -0.1
        ...
        # ============= HEALTH REWARDS =============
        if plant.health > 80:
            reward += 1.0  # Strong positive for excellent health
        elif plant.health > 60:
            reward += 0.5
        elif plant.health > 40:
            reward += 0.1
        elif plant.health < 30:
            reward -= 1.0  # Strong penalty for critical health
        elif plant.health < 20:
            reward -= 2.0
        
        # ============= GROWTH REWARDS =============
        # Encourage growth progress
        if plant.growth_stage > 0:
            reward += plant.growth_stage * 0.3
        
        # Bonus for reaching new growth stage
        if plant.growth_stage >= 1 and plant.growth_stage < 1.1:
            reward += 2.0  # Reached seedling stage
        elif plant.growth_stage >= 2 and plant.growth_stage < 2.1:
            reward += 3.0  # Reached small plant stage
        elif plant.growth_stage >= 2.9:
            reward += 5.0  # Reached mature stage
        ...
        # ============= ACTION GUIDANCE =============
        # Clear rewards for good actions
        if action == "water":
            if plant.water < 2.0:
                reward += 2.0  # Great! Plant needed water
            elif plant.water < 3.0:
                reward += 0.8  # Good timing
            elif plant.water > 4.5:
                reward -= 1.0  # Bad! Overwatering
            else:
                reward += 0.1  # Neutral
        
        elif action == "fertilize":
            if plant.soil < 2.0:
                reward += 2.0  # Great! Plant needed nutrients
            elif plant.soil < 3.0:
                reward += 0.8  # Good timing
            elif plant.soil > 4.5:
                reward -= 1.0  # Bad! Over-fertilizing
            else:
                reward += 0.1
        
        elif action == "prune":
            if plant.health < 50:
                reward += 2.5  # Great! Saved unhealthy plant
            elif plant.health < 70:
                reward += 1.0  # Good preventive care
    ...
    def _get_episode_bonus(self):
        """Large bonus for successful episode completion"""
        avg_health = np.mean([p.health for p in self.plants])
        avg_growth = np.mean([p.growth_stage for p in self.plants])
        
        bonus = 0
        
        # Health-based completion bonus
        if avg_health > 80:
            bonus += 10.0  # Excellent garden!
        elif avg_health > 70:
            bonus += 5.0   # Good garden
        elif avg_health > 60:
            bonus += 2.0   # Decent garden
        
        # Growth-based bonus
        if avg_growth > 2.5:
            bonus += 8.0   # Excellent growth!
        elif avg_growth > 2.0:
            bonus += 4.0   # Good growth
        elif avg_growth > 1.5:
            bonus += 1.0   # Some growth
        
        # Perfect garden bonus
        if avg_health > 85 and avg_growth > 2.8:
            bonus += 15.0  # Perfect garden achievement!
    ...
    def get_action_mask(self):
        ...
        # Future enhancement: mask actions that would be harmful
